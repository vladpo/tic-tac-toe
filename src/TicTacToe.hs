...
setSquare [s0, s1, s2] 0 p =
    if s0 /= E then ([s0, s1, s2], p)
    else ([sign(p), s1, s2], nextPlayer(p))
setSquare [s0, s1, s2] 1 p =
    if s1 /= E then ([s0, s1, s2], p)
    else ([s0, sign(p), s2], nextPlayer(p))
setSquare [s0, s1, s2] 2 p =
    if s2 /= E then ([s0, s1, s2], p)
    else ([s0, s1, sign(p)], nextPlayer(p))
setSquare r _ p = (r, p)
...
move [r0, r1, r2] p (M 1 y) =
    ([r0, nr, r2], np)
        where (nr, np) = setSquare r1 y p
move [r0, r1, r2] p (M 2 y)=
    ([r0, r1, nr], np)
...
allNextBoards p b = map f $ allMoves b
  where f = fst . move b p

allReactionsByMove :: Player -> Board -> [(Move, [Board])]
allReactionsByMove p b = foldl f [] $ allNextBoardsByMove p b
  where
    f = \bs t -> (fst t,  (allNextBoards . nextPlayer) p $ snd t):bs
